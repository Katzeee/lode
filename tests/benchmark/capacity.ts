/**
 * BlockEngine Capacity Benchmark
 * Run: node_modules/.bin/tsx tests/benchmark/capacity.ts
 *
 *   USER    — engine.createBlock + engine.replaceDeltas one-by-one
 *             (each call → 1 Loro commit)
 *             Models: real user interactions
 *
 *   BATCHED — BlockDoc.transact() wrapping all creates + text
...
import { getDeltaLength, textToDelta } from "../../src/delta/utils.js";
...
function makeText(seed: number, chars: number): string {
  const words = ["outline","note","task","idea","project","meeting","review",
    "plan","update","check","important","action","decision","question"];
  const out: string[] = [];
  let len = 0, i = seed;
  while (len < chars) { const w = words[i++ % words.length]; out.push(w); len += w.length + 1; }
  return out.join(" ").slice(0, chars);
}

function buildTree(blockCount: number, nestRatio: number): { rootIds: BlockId[]; allIds: BlockId[] } {
...
function runUser(blockCount: number, avgChars: number, nestRatio: number) {
  ...
  engine.mount();
  const rootCount = Math.ceil(blockCount * (1 - nestRatio));
  const rootIds: BlockId[] = [];
  const allIds: BlockId[] = [];

  const t0 = performance.now();
  for (let i = 0; i < rootCount; i++) {
    const id = engine.createBlock(); rootIds.push(id); allIds.push(id);
  }
  for (let i = rootCount; i < blockCount; i++) {
    allIds.push(engine.createBlock(rootIds[i % rootIds.length]));
  }
  ...
  const e2 = new BlockEngine({ initialBytes: snap }); e2.mount();
  const tImport = performance.now() - t1;

  engine.unmount(); e2.unmount();
...
function runBatched(blockCount: number, avgChars: number, nestRatio: number) {
  ...
  doc.transact(() => {
    for (let i = 0; i < rootCount; i++) {
      const id = doc.createBlock(); rootIds.push(id); allIds.push(id);
    }
    for (let i = rootCount; i < blockCount; i++) {
      allIds.push(doc.createBlock(rootIds[i % rootIds.length]));
    }
    for (let i = 0; i < allIds.length; i++) {
...
function runTyping(blockCount: number, avgChars: number, nestRatio: number) {
  ...
  for (let i = 0; i < allIds.length; i++) {
    ...
    const node = tree.getNodeByID(allIds[i] as any);
    if (!node) continue;
    const loroText = node.data.get("content");
    if (!(loroText instanceof LoroText)) continue;
    for (let ci = 0; ci < text.length; ci++) loroText.insert(ci, text[ci]);
    loroDoc.commit();
  }
  const tBuild = performance.now() - t0;
...
type Row = { blocks: number; chars: number; mode: string; tBuild: number; tImport: number; snapBytes: number };

async function main() {
  const SEP = "─".repeat(88);
  console.log("\n" + "═".repeat(88));
  console.log("  BlockEngine Capacity Benchmark   |   Node.js " + process.version);
  ...
  console.log(H("Blocks × chars", 22) + H("Mode", 10) + H("Raw text", 12) +
    H("Snapshot", 12) + H("Ratio", 8) + H("Build", 10) + H("Import", 10));
  console.log(SEP);

  const sorted = [...results].sort((a, b) =>
    a.mode.localeCompare(b.mode) || a.blocks - b.blocks);
  ...
  const largest = bResults[bResults.length - 1];
  ...
  for (const r of bResults) {
    ...
    console.log(`  ${String(r.blocks).padStart(7)}: ${fmtMs(r.tBuild).padStart(9)}  (${µs}µs/block)   ${bpc} bytes/char   import ${fmtMs(r.tImport)}`);
  }
  ...
  for (const scenario of [
    ...
    console.log(`  ${scenario.label}:  raw ${fmt(rawText).padStart(8)}  snap ~${fmt(snap).padStart(9)}  import ~${fmtMs(importMs)}`);
  }

  const tResults = results.filter(r => r.mode === "TYPING ");
  if (tResults.length >= 1) {
    const lt = tResults[tResults.length - 1];
  ...
  if (uResults.length >= 2) {
    ...
    for (const r of uResults) {
      const µs = (r.tBuild / r.blocks * 1000).toFixed(0);
      console.log(`  ${String(r.blocks).padStart(6)} blocks: ${fmtMs(r.tBuild).padStart(9)}  (${µs}µs/block)`);
    }
    const u0 = uResults[0], u1 = uResults[uResults.length - 1];
