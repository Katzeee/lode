...
type WorkerScope = Readonly<{ postMessage(message: unknown): void }>;

const workerScope = globalThis as unknown as WorkerScope;
let sequence = 0;
const pending = new Map<
  string,
  Readonly<{ reject(error: Error): void; resolve(value: unknown): void }>
>();
...
function request(operation: NativeStorageOperation): Promise<unknown> {
  ...
  return new Promise((resolve, reject) => {
    ...
    workerScope.postMessage({ type: 'native-storage-request', id, operation });
  });
}
