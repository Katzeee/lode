...
The product top bar exposes a **Review design system** entry. It opens the renderer's `#/design-system` catalog, where Overview, Foundations, Components, Patterns, Templates & pages, and Review have stable routes backed by one catalog shared with Mobile. Returning to the product does not recreate the desktop host or change Engine state. The adjacent **Legal** entry exposes the complete HarmonyOS Sans attribution and license.

For continuous design work, run `npm run dev:desktop-web` from the repository root and open `http://127.0.0.1:4173/#/design-system`. The development server bundles the same renderer entry and real catalog components used by Electron, but omits the host, daemon, and desktop bridge. It is the primary surface for responsive, theme, accessibility, and interaction review; the packaged Electron verification remains the authority for preload, IPC, native-window, DPI, and packaged-asset behavior.
...
`build:desktop` builds the required workspace packages, bundles the main, preload, renderer, daemon, and licensed design assets without development sourcemaps, invokes Forge, and prints the absolute artifact path. On the validated x64 host the runnable directory is `apps/desktop/out/Lode-win32-x64`; an arm64 Windows host selects the matching architecture. `verify:desktop` launches `Lode.exe` itself, not a development server, and exercises initialization, persistence, cold start, daemon reuse, stale endpoint recovery, startup failure, and owned-process cleanup in a generated temporary Home. It also verifies that the packaged HarmonyOS Sans file is byte-identical to the official source asset. Build output is ignored and can be regenerated from the checkout.
