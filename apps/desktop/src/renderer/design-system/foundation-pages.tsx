...
export function GeometryPage() {
  return (
    <>
      ...
      >
        <div className="grid grid-cols-[1fr_1.2fr_0.8fr_0.8fr] gap-3 pb-2 text-caption font-semibold tracking-widest text-muted-foreground uppercase">
          <span>Class</span>
        ...
        {windowClasses.map((windowClass) => (
          <div className="grid grid-cols-[1fr_1.2fr_0.8fr_0.8fr] gap-3 py-2.5 text-body" key={windowClass.id}>
            <span className="font-medium">{windowClass.id}</span>
            <span className="font-mono text-label text-muted-foreground">{windowClass.range}</span>
            <span className="text-muted-foreground">{windowClass.columns}</span>
            <span className="text-muted-foreground">{windowClass.gutter}px</span>
          </div>
        ))}
      </Specimen>
      <Specimen
