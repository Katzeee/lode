...
| `@lode/engine`     | Transport-free core, layered: `core` (block tree, text, props, history, CRDT sync primitives); `persistence` (storage primitives); `domain` + `domain/model` (product semantics + value types); `bundle` (built-in schema); `event` + `session` (notifications/sessions); `services` (RPC adapters); `runtime` (App/Component composition + in-process sync core). |
