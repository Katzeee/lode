...
// in session-rpcs (they reach the identity store + notification manager directly); the composition
// root merges them + auth-wraps the lot (wrapCommands).
 ...
 *  RPCs. (The proto `LodeCommands` service descriptor is a separate thing — `@lode/protocol/proto`.) */
export type Commands = WrappedCommands<ReturnType<typeof createCommands> & SessionRpcs>;
