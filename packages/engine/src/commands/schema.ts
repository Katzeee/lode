...
export function createSchemaHandlers(ctx: CommandDeps) {
  ...
  return {
    createSchema: authed(async (req: CreateSchemaRequest, caller): Promise<NodeOccurrenceRef> => {
      const identity = await runMutation(ctx, caller, req.workspaceId, (engine) =>
        createSchema(engine, req.name, req.parentOccurrenceId),
      );
    ...
    applySchema: authed(async (req: ApplySchemaRequest, caller): Promise<SchemaChangeResult> => {
      return toResult(
        await runMutation(ctx, caller, req.workspaceId, (engine) =>
          applySchema(engine, req.targetOccurrenceId, req.schemaNodeId),
        ),
      );
    }),
    ...
    removeSchema: authed(async (req: RemoveSchemaRequest, caller): Promise<SchemaChangeResult> => {
      return toResult(
        await runMutation(ctx, caller, req.workspaceId, (engine) =>
          removeSchema(engine, req.targetOccurrenceId, req.schemaNodeId),
        ),
      );
    }),
    ...
    reconcileSchema: authed(
      async (req: ReconcileSchemaRequest, caller): Promise<SchemaChangeResult> => {
        return toResult(
          await runMutation(ctx, caller, req.workspaceId, (engine) =>
            reconcileSchema(engine, req.targetOccurrenceId),
          ),
        );
      },
    ),
