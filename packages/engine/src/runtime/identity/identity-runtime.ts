...
/**
 * Home-level identity runtime: the Actor Vault plus the Peer identity. The
 * vault holds every Actor this Home can act as; unlocking loads Ed25519 seeds
...
export class IdentityRuntime {
  ...
  static async open(dataRoot: string | undefined): Promise<IdentityRuntime> {
    const vault = await VaultStore.open(dataRoot === undefined ? undefined : join(dataRoot, "identity", "vault.json"));
    const peer = await loadOrCreatePeerMaterial(
      dataRoot === undefined ? undefined : join(dataRoot, "identity", "peer.json"),
    );
    return new IdentityRuntime(vault, peer);
  }
