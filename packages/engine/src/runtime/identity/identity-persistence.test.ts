...
describe("identity persistence", () => {
  it("fails closed when an existing Actor Vault is corrupt", async () => {
    ...
    const identity = await IdentityRuntime.open(dataRoot);
    ...
    await expect(IdentityRuntime.open(dataRoot)).rejects.toThrow("Cannot load Actor Vault");
  ...
  it("fails closed instead of replacing an existing corrupt Peer identity", async () => {
    const dataRoot = await temporaryRoot();
    const first = await IdentityRuntime.open(dataRoot);
    const originalPeerId = first.peer().peerId;
    ...
    await expect(IdentityRuntime.open(dataRoot)).rejects.toThrow("Cannot load Peer identity");
