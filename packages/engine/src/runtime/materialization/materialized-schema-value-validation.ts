import type { ProjectionSectionName } from "../../domain/reconcile/index.js";
import { hasExactKeys, isStringArray } from "./materialized-validation-primitives.js";

export function isSchemaSectionValue(
...
function isEffectiveField(field: unknown): boolean {
  return (
    ...
    isStringArray(field.sourceSchemaIds) &&
    isStringArray(field.sourceFieldNodeIds) &&
...
function isFieldConfigCandidate(value: unknown): boolean {
  return (
    ...
    isStringArray(value.sourceSchemaIds) &&
    isStringArray(value.sourceFieldNodeIds) &&
    isStringArray(value.contributionIds)
  );
}
