...
export function materialize(generation: ProjectionGeneration): MaterializedGeneration {
  const shards = [
  ...
  return {
    header: withHeaderDigest({
      ...
      ownerCache: {
        documentId: ownerCacheDocumentId(generation.identity.generationId),
        contentDigest: canonicalDigest(generation.ownerCaches),
      },
    ...
    ownerCaches: {
      format: OWNER_CACHE_FORMAT,
      generationId: generation.identity.generationId,
      contentDigest: canonicalDigest(generation.ownerCaches),
      value: generation.ownerCaches,
    },
    shards,
...
function projectionShards(
  ...
  const entries: { section: ProjectionSection; identity: string; value: unknown }[] = [];
