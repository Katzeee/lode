...
async function readApplicationStructure(workspace: ProposalWorkspace, applicationNodeId: string) {
...
async function readApplicationEndpoints(workspace: ProposalWorkspace, applicationNodeId: string) {
...
async function readExtensions(workspace: ProposalWorkspace, perspective: "origin" | "review") {
  const result = await workspace.query({
    kind: "projection",
    workspaceId: "workspace",
    perspective,
