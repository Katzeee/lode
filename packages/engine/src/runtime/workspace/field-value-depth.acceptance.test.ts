...
async function establishFixture(workspace: ProposalWorkspace): Promise<void> {
  await publish(
    workspace,
    command("fixture", [
...
async function outlineNodeIds(workspace: ProposalWorkspace, rootNodeId: string): Promise<readonly string[]> {
  const result = await workspace.query({
    kind: "outline",
    workspaceId: "workspace",
    perspective: "origin",
...
async function acceptAllReview(workspace: ProposalWorkspace): Promise<void> {
...
async function publishHistory(
  workspace: ProposalWorkspace,
  commandValue: Parameters<ProposalWorkspace["execute"]>[0],
): Promise<void> {
