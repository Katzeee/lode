...
// The App starts components in registration order, runs their long-lived loops concurrently (each
 ...
 *  to pause or back off background work (e.g. sync ticks). The App stores + propagates the value to
...
export type Component = {
  ...
  /** A long-running loop (tick, poll, stream). Receives an AbortSignal the App aborts on stop(). A
...
export class App {
  ...
  constructor(parent?: App, options: AppOptions = {}) {
    this.parent = parent;
  ...
  // the App owns only lifecycle ordering. Throws on a duplicate name — a programming error, not a
