...
import { App, type Component, type DeviceState } from "./app.js";
...
describe("App run loops", () => {
  ...
  it("does not hang on stop if a run loop ignores the abort — bounded by runSettleMs", async () => {
    const stopped: string[] = [];
    const app = new App(undefined, { runSettleMs: 30 });
    app.register({
      name: "hung",
      start() {},
...
describe("App registration + device state", () => {
