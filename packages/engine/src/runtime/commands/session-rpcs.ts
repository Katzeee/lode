...
import { EMPTY } from "../../services/wire/empty.js";
import type { AppWorkspaceRuntime } from "../workspace/registry.js";
import type { SessionIdentity } from "../identity/session-identity.js";
import { deriveActorKeypair, mintActorIdentity } from "../identity/identity-policy.js";

// The RPCs that are NOT domain adapters — session/identity (hello, mint, getActorPublicKeys) +
// notification (subscribe/unsubscribe/listen). They live in the runtime (not services) because they
// reach the identity store + the notification hub directly, which are runtime-resident. Each handler
// declares its own auth contract: hello/mint/listen are `open` (no session — hello CREATES it, mint
// is bootstrap, listen is the pre-auth stream); getActorPublicKeys/subscribe/unsubscribe are
// `authed`.

/** Resolve the engine for `workspaceId` or throw (the existence check subscribeDoc needs). */
async function requireEngine(workspaces: AppWorkspaceRuntime, workspaceId: string) {
