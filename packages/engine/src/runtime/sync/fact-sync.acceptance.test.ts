...
import { ProposalWorkspace } from "../workspace/proposal-workspace.js";
...
describe("Fact-only production sync", () => {
  ...
  it("rejects synced Template Field discoverability that carries an Intrinsic Node Type rewrite", async () => {
    ...
    const source = await ProposalWorkspace.open({ workspaceId: "workspace", facts: sourceStore, versions });
    ...
    expect(
      rebuildGeneration("workspace", admission.snapshot, versions).generation.origin.nodes["sync-field-definition"]
        ?.intrinsicNodeType,
  ...
  it("syncs a same-Definition Template Field re-add and rejects authority smuggled through its narrower Fact", async () => {
    ...
    await target.replication.importUpdate(await sourceStore.replication.exportSnapshot());
    expect(target.admission().kind).toBe("ready");
    const synced = rebuildGeneration("workspace", target.snapshot(), versions).generation.origin;
  ...
  it("syncs a public Static Default edit and rejects Owner authority smuggled through its text Fact", async () => {
    ...
    await target.replication.importUpdate(await sourceStore.replication.exportSnapshot());
    expect(target.admission().kind).toBe("ready");
    let synced = rebuildGeneration("workspace", target.snapshot(), versions).generation.origin;
    expect(
    ...
    const sourceProjection = rebuildGeneration("workspace", sourceStore.snapshot(), versions).generation.origin;
    ...
    synced = rebuildGeneration("workspace", fault.snapshot, versions).generation.origin;
    expect(
      textAtoms(synced.nodes["static-default-sync-value"])
        .map((atom) => atom.value)
        .join(""),
  ...
  it("rejects a synced View Sort transaction that also rewrites the protected Definition Intrinsic Node Type", async () => {
    ...
    const source = await ProposalWorkspace.open({ workspaceId: "workspace", facts: sourceStore, versions });
    const target = await domainReplica("202", "bbbbbbbbbbbbbbbbbbbbbbbbbb");
    const publish = async (command: Parameters<ProposalWorkspace["execute"]>[0]): Promise<void> => {
  ...
  it("syncs a public View removal as its complete removal graph", async () => {
    ...
    const source = await ProposalWorkspace.open({ workspaceId: "workspace", facts: sourceStore, versions });
    await ProposalWorkspace.open({ workspaceId: "workspace", facts: targetStore, versions });
    ...
    const attached = rebuildGeneration("workspace", targetStore.snapshot(), versions).generation.origin;
    ...
    const generation = rebuildGeneration("workspace", targetStore.snapshot(), versions).generation;
  ...
  it("syncs typed View options and rejects a compensation transaction that smuggles Field Definition ownership", async () => {
    ...
    await targetStore.replication.importUpdate(await sourceStore.replication.exportSnapshot());
    const synced = rebuildGeneration("workspace", targetStore.snapshot(), versions).generation.origin;
...
async function runVisibilityTopology(edges: readonly (readonly [number, number])[]) {
  ...
  const setup = await ProposalWorkspace.open({
    workspaceId: "workspace",
  ...
  const workspaces = await Promise.all(
    stores.map((facts) => ProposalWorkspace.open({ workspaceId: "workspace", facts, versions })),
  );
  ...
  const generation = rebuildGeneration("workspace", required(snapshots[0], "visibility snapshot"), versions).generation;
