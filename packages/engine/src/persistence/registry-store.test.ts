...
import { SqliteRegistryStore, type RegistryStore } from "./registry-store.js";
...
beforeEach(async () => {
  ...
  store = await SqliteRegistryStore.open(tempDir);
});
...
describe("RegistryStore peerId", () => {
  ...
  it("different dataRoots get different peerIds", async () => {
    ...
    try {
      const other = await SqliteRegistryStore.open(otherDir);
