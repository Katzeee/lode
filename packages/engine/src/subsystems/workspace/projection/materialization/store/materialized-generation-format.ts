...
import { MaterializedGenerationCorruptError } from "./errors.js";
...
export function directoryRoot(header: GenerationHeader, root: MaterializedDatasetRoot): DirectoryRoot {
  ...
  if (!found) {
    throw new MaterializedGenerationCorruptError("Published materialized dataset directory root is absent");
  }
  return found;
}
