...
} from "./store/materialized-dataset.js";
import { projectionMaterializedDataset } from "./projection-materialized-dataset.js";
import { projectionLookupIndexEntries } from "./materialized-projection-index.js";
...
describe("materialized datasets", () => {
  it("indexes each materialized Field owner once", () => {
    const empty = rebuildGeneration("workspace", { facts: [], frontier: {} }, CURRENT_PROJECTION_VERSIONS).origin;
    const entries = projectionLookupIndexEntries({
      ...empty,
      materializedFields: {
        owner: [
          {
    ...
    expect(entries.find((entry) => entry.section === "nodeIdsByFieldDefinition")).toEqual({
