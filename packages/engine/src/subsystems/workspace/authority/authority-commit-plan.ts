...
import type { AuthorityAdmissionPolicy, AuthorityCommit } from "./authority-contract.js";
import {
...
export function planAuthorityCommit(
  ...
  if (
    ...
    throw new AuthorityAdmissionError("Authority commit requires non-empty Fact transactions");
  }
  ...
  if (candidate.kind !== "ready") {
    throw new AuthorityAdmissionError(candidate.fault ?? "Local Fact batch did not admit completely");
  }
  ...
  return { kind: "append", receipt, records, admission };
}
...
function assertLocalFactsAdmitted(planned: Admission, policy: Admission, facts: readonly Fact[]): void {
  ...
  if (
    ...
    throw new AuthorityAdmissionError(policy.fault ?? "Local Fact transactions violate admission policy");
  }
}
