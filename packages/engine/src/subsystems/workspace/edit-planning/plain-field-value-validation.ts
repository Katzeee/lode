...
export function validatePlainOrOptionsValue(
  ...
  if (datatypes.length > 1) {
    throw new EditPlanningRejection("Field Definition must have an unconflicted Datatype configuration");
  }
  ...
  if (datatypeNodeId !== FIELD_DATATYPE_NODE_IDS.plain && datatypeNodeId !== FIELD_DATATYPE_NODE_IDS.options) {
    throw new EditPlanningRejection("Typed Field Values require a typed value edit");
  }
  const cardinalities = configurations.filter((configuration) => configuration.kind === "cardinality");
  if (cardinalities.length > 1) {
    throw new EditPlanningRejection("Field Definition must have an unconflicted Cardinality configuration");
  }
  ...
  if (
    ...
    throw new EditPlanningRejection("Single-value Field already has an authored value");
  }
}
