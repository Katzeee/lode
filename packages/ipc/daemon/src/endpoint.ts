...
export function parseEndpoint(endpoint: string): ParsedEndpoint {
  ...
  switch (url.protocol) {
    ...
    default:
      throw new Error(
        `Unsupported endpoint protocol (expected tcp://, unix://, or pipe://): ${endpoint}`,
      );
  }
}
...
/** A raw socket to the endpoint — for liveness probing (`probeEndpoint`). */
export function connectSocket(endpoint: string): net.Socket {
  return rawSocket(parseEndpoint(endpoint));
}
...
/** The dial descriptor the client transport consumes: either a plain TCP URL or a pre-resolved
 *  socket connection (UDS / Windows named pipe) with a placeholder HTTP/2 authority. The client is
 *  scheme-agnostic — it just runs gRPC/HTTP-2 over whatever this yields. Structurally matches
 *  `@lode/client`'s `SocketDial` (the field names are the contract between daemon and client); the
 *  daemon owns scheme resolution so the client doesn't have to. */
export type EndpointDial =
  | { readonly tcpUrl: string }
  | { readonly authority: string; readonly createConnection: () => net.Socket };
