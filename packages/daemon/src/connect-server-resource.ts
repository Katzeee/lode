...
export class ConnectServerResource {
  ...
  private closeConnections: () => Promise<void> = async () => {};
  private boundPort = 0;
  ...
  private async failStart(primary: unknown): Promise<never> {
    try {
      await this.close();
    } catch (cleanupError) {
      const failure = new AggregateError(
        [toError(primary), toError(cleanupError)],
        "Daemon listener setup and cleanup failed",
        {
          cause: primary,
        },
      );
  ...
  async close(): Promise<void> {
    ...
    if (failures.length > 1) {
      throw new AggregateError(failures, "Daemon listener failed to close cleanly");
    }
  }
}
